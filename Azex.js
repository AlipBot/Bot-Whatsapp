...
const menubot = require('./menu/azex229.js')
...
module.exports = Zeeone = async (Zeeone, Ofc) => {
	try {
		...
		const time = moment.tz('Asia/Kuala_Lumpur').format('DD/MM HH:mm:ss')
		...
		const timi = moment.tz('Asia/Kuala_Lumpur').add(30, 'days').calendar();
		const timu = moment.tz('Asia/Kuala_Lumpur').add(20, 'days').calendar();
...
Minat? Pm wa.me/${owner_number}`,
       ...
       async function sendFileFromUrl(from, url, caption, Ofc, men) {
        ...
        const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Kuala_Lumpur').format('HH:mm:ss'), "gold"), color('Downloading sticker...'))
				...
				download(url, namea, async function () {
					...
					console.log(color(time, 'magenta'), color(moment.tz('Asia/Kuala_Lumpur').format('HH:mm:ss'), "gold"), color('Succes send sticker...'))
					});
					});
				});
			}
