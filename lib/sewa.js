    ...
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                ket.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    ket.groupLeave(_dir[position].id)
                    .then(() => {
